...
Each set included, in a random order, the 3 blocks of illusion types, in which trials were separated by a fixation cross, temporally (uniformly sampled duration of 500 - 1000s) and spatially jittered (around the centre of the screen in a radius of a 1 cm) to attenuate its potential usefulness as a reference point. 
...
As a secondary exploratory outcome, the Inverse Efficiency Score (IES, Townsend & Ashby, 2014) was computed. This metric incorporates both speed and accuracy by dividing the mean reaction time of correct responses by the proportion of correct responses, separately for each illusion.
...
table |>
  ...
  flextable::add_footer_lines("Note. These values present mean and standard deviations.")
...
The 10 items forming the PCS score yielded accecptable consistency ($\alpha = 0.78$).
