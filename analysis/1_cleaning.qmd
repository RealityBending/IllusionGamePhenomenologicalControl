...
data_ppt <- read.csv("C:/Users/olive/Documents/University/Year 3 (Placement Year)/REBEL/IllusionGamePhenomenologicalControl/Data/rawdata_participants.csv")

data_ig <- read.csv("C:/Users/olive/Documents/University/Year 3 (Placement Year)/REBEL/IllusionGamePhenomenologicalControl/Data/rawdata_illusion.csv") |>
    mutate(
...
The initial sample consisted of `r report::report_participants(data_ppt, age="Age", gender="Gender", education="Education")`, for a total trial number of `r nrow(data_ig)`.
...
```{r}
data_ppt <- filter(data_ppt, !Participant %in% outliers_full)
...
```{r}
ig_ex_tib <- bind_rows(
  tibble(Participant = outliers_full, "Extent of Exclusion" = "Participant-level"),
  tibble(Participant = outliers_half, "Extent of Exclusion" = "Block-level")
)

ig_ex_tib |> 
  knitr::kable(caption = "Exclusions From Illusion Game") |> 
...
```{r}
ig_ex_bar <- ig_ex_tib |> 
  ggplot(aes(x = `Extent of Exclusion`, fill = `Extent of Exclusion`)) +
  geom_bar(width = 0.6) +
  scale_x_discrete(labels = scales::label_wrap(15)) +
  scale_y_continuous(breaks = seq(0, 10, by = 2)) +
  labs(
    title = "Exclusions From Illusion Game",
    x = "Extent of Exclusion",
    y = "Count",
  ) +
  theme_minimal() +
  guides(fill = "none")
...
```{r}
n_before <- nrow(data_ig)
data_ig <- filter(data_ig, RT > 0.2 & RT < 5)
n_removed <- n_before - nrow(data_ig)
```
...
We removed `r length(outliers_pcs)` (`r insight::format_percent(length(outliers_pcs) / nrow(data_ppt))`) participants that either did not report hearing "Hello" at the beginning of the experiment, responded that no balls were presented on the screen, or pressed the space bar fewer than 5 times when asked to do so.
...
p_eth <- data_ppt |>
  ...
  mutate(Freq = format_percent(n() / nrow(data_ppt), digits=1), .by=Ethnicity) |> 
...
write.csv(data_ppt, "C:/Users/olive/Documents/University/Year 3 (Placement Year)/REBEL/IllusionGamePhenomenologicalControl/Data/data_participants.csv", row.names = FALSE)
write.csv(data_ig, "C:/Users/olive/Documents/University/Year 3 (Placement Year)/REBEL/IllusionGamePhenomenologicalControl/Data/data_illusion.csv", row.names = FALSE)
```
