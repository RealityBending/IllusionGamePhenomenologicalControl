...
illusion_ebbinghaus_err <- brms::brm(formula,
  data = data,
  refresh = 100,
  normalize = FALSE
)

save(illusion_ebbinghaus_err, file = "models/illusion_ebbinghaus_err.Rdata")
```
...
```{r}
load("models/illusion_ebbinghaus_err.Rdata")
...
p_ebbinghaus_err <- plot_model_errors(data, illusion_ebbinghaus_err, gam_ebbinghaus_err)
p_ebbinghaus_err
```
...
```{r}
performance::performance(illusion_ebbinghaus_err, metrics = c("R2", "ICC")) |>
  display()
```
...
illusion_ebbinghaus_rt <- brms::brm(formula,
  data = data,
  refresh = 100,
  init = 0,
  normalize = FALSE
...
save(illusion_ebbinghaus_rt, file = "models/illusion_ebbinghaus_rt.Rdata")
```
...
```{r}
load("models/illusion_ebbinghaus_rt.Rdata")
load("models/gam_ebbinghaus_rt.Rdata")
```

```{r}
plot_model_rt <- function(data, model, gam) {
 pred <- estimate_relation(
  model,
  at = c("Illusion_Effect", "Illusion_Difference", "Illusion_Strength"),
  length = c(3,100)
)
...
p_ebbinghaus_rt <- plot_model_rt(data, illusion_ebbinghaus_rt, gam_ebbinghaus_rt)
p_ebbinghaus_rt
```
...
```{r}
performance::performance(illusion_ebbinghaus_rt, metrics = c("R2", "ICC")) |>
  display()
```
...
plot_ppcheck(illusion_ebbinghaus_rt, gam_ebbinghaus_rt)
```
...
plot_all <- function(data, p_err, p_rt, question="") {
  ...
  dat <- data |>
    ...
    filter(
      ...
      Response %in% c("left", "up")
    ) |>
    select(File, Illusion_Strength, Illusion_Difference)
  ...
  img_leftdown <- paste0("../experiment/stimuli/", img_leftdown, ".png") |>
    png::readPNG() |>
    grid::rasterGrob(interpolate = TRUE) |>
    patchwork::wrap_elements()
  img_rightdown <- paste0("../experiment/stimuli/", img_rightdown, ".png") |>
    png::readPNG() |>
    grid::rasterGrob(interpolate = TRUE) |>
    patchwork::wrap_elements()
  img_leftup <- paste0("../experiment/stimuli/", img_leftup, ".png") |>
    png::readPNG() |>
    grid::rasterGrob(interpolate = TRUE) |>
    patchwork::wrap_elements()
  img_rightup <- paste0("../experiment/stimuli/", img_rightup, ".png") |>
    png::readPNG() |>
    grid::rasterGrob(interpolate = TRUE) |>
    patchwork::wrap_elements()
...
illusion_mullerlyer_err <- brms::brm(formula,
  data = data,
  refresh = 100,
  normalize = FALSE
)

save(illusion_mullerlyer_err, file = "models/illusion_mullerlyer_err.Rdata")
```
...
```{r}
load("models/illusion_mullerlyer_err.Rdata")
load("models/gam_mullerlyer_err.Rdata")
```

```{r}
p_mullerlyer_err <- plot_model_errors(data, illusion_mullerlyer_err, gam_mullerlyer_err)
p_mullerlyer_err
```
...
```{r}
performance::performance(illusion_mullerlyer_err, metrics = c("R2", "ICC")) |>
  display()
```
...
illusion_mullerlyer_rt <- brms::brm(formula,
  data = data,
  refresh = 100,
  init = 0,
  normalize = FALSE
...
save(illusion_mullerlyer_rt, file = "models/illusion_mullerlyer_rt.Rdata")
```
...
```{r}
load("models/illusion_mullerlyer_rt.Rdata")
load("models/gam_mullerlyer_rt.Rdata")
```

```{r}
p_mullerlyer_rt <- plot_model_rt(data, illusion_mullerlyer_rt, gam_mullerlyer_rt)
p_mullerlyer_rt
```
...
```{r}
performance::performance(illusion_mullerlyer_rt, metrics = c("R2", "ICC")) |>
  display()
```

```{r}
plot_ppcheck(illusion_mullerlyer_rt, gam_mullerlyer_rt)
```
...
illusion_verticalhorizontal_err <- brms::brm(formula,
  data = data,
  refresh = 100,
  normalize = FALSE
)

save(illusion_verticalhorizontal_err, file = "models/illusion_verticalhorizontal_err.Rdata")
```

#### Model Inspection

```{r}
load("models/illusion_verticalhorizontal_err.Rdata")
...
p_verticalhorizontal_err <- plot_model_errors(
  data,
  illusion_verticalhorizontal_err,
...
```{r}
performance::performance(illusion_verticalhorizontal_err, metrics = c("R2", "ICC")) |>
  display()
```
...
illusion_verticalhorizontal_rt <- brms::brm(formula,
  data = data,
  refresh = 100,
  init = 0,
  normalize = FALSE
...
save(illusion_verticalhorizontal_rt, file = "models/illusion_verticalhorizontal_rt.Rdata")
```
...
```{r}
load("models/illusion_verticalhorizontal_rt.Rdata")
...
p_verticalhorizontal_rt <- plot_model_rt(
  data,
  illusion_verticalhorizontal_rt,
...
```{r}
performance::performance(illusion_verticalhorizontal_rt, metrics = c("R2", "ICC")) |>
  display()
```

```{r}
plot_ppcheck(illusion_verticalhorizontal_rt, gam_verticalhorizontal_rt)
```
...
for(m in c("ebbinghaus", "mullerlyer", "verticalhorizontal")) {
  for(i in c("err", "rt")) {
    load(paste0("models/illusion_", m, "_", i, ".Rdata"))
  }
}
```
...
ebbinghaus_err <- get_scores(illusion_ebbinghaus_err, "Ebbinghaus")
ebbinghaus_rt <- get_scores(illusion_ebbinghaus_rt, "Ebbinghaus")
mullerlyer_err <- get_scores(illusion_mullerlyer_err, "MullerLyer")
mullerlyer_rt <- get_scores(illusion_mullerlyer_rt, "MullerLyer")
verticalhorizontal_err <- get_scores(illusion_verticalhorizontal_err, "VerticalHorizontal")
verticalhorizontal_rt <- get_scores(illusion_verticalhorizontal_rt, "VerticalHorizontal")
```
...
write.csv(scores, "../data/scores_illusio.csv", row.names = FALSE)
```
